 ...
 */
public class ListenersFactory<L> implements InvocationHandler {
    ...
    private ListenersFactory(Class<L> listenerClass, ListenersParent<L> parent) {
        this.parent = parent;
        try {
    ...
    public static IListeners<?> of(Class<?> listenerClass, ListenersParent<?> parent) {
        return (IListeners<?>) Proxy.newProxyInstance(
                ListenersFactory.class.getClassLoader(),
                new Class[]{listenerClass, IListeners.class},
                new ListenersFactory(listenerClass, parent));
    }
